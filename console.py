...
class HBNBCommand(cmd.Cmd):
    ...
    def do_quit(self, line):
        """Command interpreter to exit/quit program"""
        return True

    do_EOF = do_quit

    def do_create(self, line):
        """Command interpreter to create object"""
        if not len(line):
    ...
    def do_show(self, line):
        """Command interpreter to show object"""
        if not len(line):
            print("** class name missing **")
            return
        strings = split(line)
    ...
    def do_all(self, line):
        """Command interpreter to print all"""
        if not len(line):
        ...
        if strings[0] not in HBNBCommand.classes:
            print("** class doesn't exist **")
            return
        print([obj for obj in storage.all().values() if
               strings[0] == type(obj).__name__])

    def do_update(self, line):
        """Command interpreter to update object"""
        if not len(line):
            print("** class name missing **")
            return
        strings = split(line)
    ...
    def stripper(self, st):
        """Command interpreter strips line"""
    ...
    def dict_strip(self, line):
        """Strips while checking for dictionary"""
        newstring = st[st.find("(")+1:st.rfind(")")]
        try:
